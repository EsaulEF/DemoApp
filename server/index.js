...
app.post("/exchange/restaurant/reservations/email", async (req, res) => {
  ...
  console.log('request ---- signature ',timestamp+payload,' ', generateSignature(timestamp+payload, signingSecret), 'singing secret ', signingSecret, 'timestamp ', timestamp,'signature ', signature,  'headers ', req.headers)
  console.log('------*--*-*-*-**-*- ',signature === generateSignature(timestamp+payload, signingSecret));
